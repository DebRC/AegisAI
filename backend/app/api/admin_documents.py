...
def list_documents(offset: int = Query(0, ge=0), limit: int = Query(25, ge=1, le=100), status_filter: DocumentStatus | None = Query(None, alias="status"), uploader_user_id: int | None = Query(None, ge=1), service: AdminDocumentService = Depends(get_admin_document_service), _: User = Depends(require_administration_permission(PermissionCode.DOCUMENTS_MANAGE)), context: TenantContext | None = Depends(get_current_tenant_context)) -> AdminDocumentListResponse:
    ...
    arguments = dict(offset=offset, limit=limit, status=status_filter, uploader_user_id=uploader_user_id)
    if tenant_id is not None: arguments["tenant_id"] = tenant_id
    page = service.list_documents(**arguments)
...
def get_document(document_id: int, service: AdminDocumentService = Depends(get_admin_document_service), _: User = Depends(require_administration_permission(PermissionCode.DOCUMENTS_MANAGE)), context: TenantContext | None = Depends(get_current_tenant_context)) -> AdminDocumentResponse:
    try:
        tenant_id = getattr(getattr(context, "tenant", None), "id", None)
        return _response(service.get_document(document_id, tenant_id=tenant_id)) if tenant_id is not None else _response(service.get_document(document_id))
