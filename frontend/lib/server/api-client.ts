...
export const aegisApi = {
  ...
  createRole(accessToken: string, name: string, description: string | null): Promise<unknown> { return backendRequest("/rbac/roles", { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ name, description }) }); },
  grantRolePermission(accessToken: string, roleId: number, permissionId: number): Promise<unknown> { return backendRequest(`/rbac/roles/${roleId}/permissions/${permissionId}`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}` } }); },
  revokeRolePermission(accessToken: string, roleId: number, permissionId: number): Promise<void> { return backendRequest(`/rbac/roles/${roleId}/permissions/${permissionId}`, { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } }); },
